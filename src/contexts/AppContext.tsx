...
interface AppActions {
  ...
  updateExpense: (expenseId: string, updated: Omit<Expense, 'id'>) => Promise<void>;
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const addExpense = useCallback(async (expense: Omit<Expense, 'id'>) => {
    ...
    setState((s) => ({ ...s, expenses: [...s.expenses, newExpense], isSyncing: true }));
  ...
  const updateExpense = useCallback(async (expenseId: string, updated: Omit<Expense, 'id'>) => {
    const ssId = getSsId();
    if (!ssId) return;
    ...
    setState((s) => ({
      ...s,
      expenses: s.expenses.map((e) => (e.id === expenseId ? updatedExpense : e)),
      isSyncing: true
    }));

    try {
      await sheetsApi.updateExpenseRow(ssId, idx, updatedExpense, current.members);
      setState((s) => ({ ...s, isSyncing: false }));
    } catch (err) {
      setState((s) => ({
        ...s,
  ...
  const addMember = useCallback(async (name: string) => {
    ...
    try {
      ...
      const newProfiles = [
        ...current.members.map((m) => current.memberProfiles[m] || { name: m, email: '', photoUrl: '' }),
        { name, email: '', photoUrl: '' }
      ];
  ...
  const renameMember = useCallback(async (oldName: string, newName: string) => {
    const ssId = getSsId();
    ...
    setState((s) => ({
      ...
      expenses: s.expenses.map((e) => {
        ...
        return { ...e, paidBy: e.paidBy === oldName ? trimmed : e.paidBy, splits: newSplits };
      }),
      memberProfiles: (() => {
        const p = { ...s.memberProfiles };
        if (p[oldName]) { p[trimmed] = { ...p[oldName], name: trimmed }; delete p[oldName]; }
        return p;
      })(),
      isSyncing: true
    }));
    ...
    try {
      await sheetsApi.renameMemberColumn(ssId, current.members, oldName, trimmed);
      const updatedProfiles: MemberInfo[] = current.members.map((m) => {
        ...
        return current.memberProfiles[m] || { name: m, email: '', photoUrl: '' };
      });
  ...
  const linkMemberToGoogle = useCallback(async (memberName: string) => {
    ...
    try {
      ...
      const current = stateRef.current;
      const updatedProfiles: Record<string, MemberInfo> = { ...current.memberProfiles };
      for (const key of Object.keys(updatedProfiles)) {
        if (updatedProfiles[key].email === user.email && key !== memberName) {
          updatedProfiles[key] = { ...updatedProfiles[key], email: '', photoUrl: '' };
        }
      }
      ...
      const allProfiles: MemberInfo[] = current.members.map((m) =>
        updatedProfiles[m] || { name: m, email: '', photoUrl: '' }
      );
  ...
  const settleUp = useCallback(async (from: string, to: string, amount: number) => {
    ...
    setState((s) => ({ ...s, expenses: [...s.expenses, settlement], isSyncing: true }));
  ...
  const importExpenses = useCallback(async (newExpenses: Expense[], newMembers: string[]) => {
    ...
    try {
      ...
      await sheetsApi.writeAllExpenses(ssId, allExpenses, allMembers, current.currency);
      setState((s) => ({
        ...s,
        members: allMembers,
  ...
  const actions = useMemo<AppActions>(() => ({
    ...
    selectSpreadsheet, loadData, addExpense, updateExpense, deleteExpense,
    addMember, renameMember, linkMemberToGoogle, settleUp, renameSheet,
    importExpenses, disconnect
  ]);
